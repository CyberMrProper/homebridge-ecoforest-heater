...
module.exports = function(homebridge){
  ...
  homebridge.registerAccessory("homebridge-ecoforest-heater", "EcoforestHeater", EcoforestHeater);
};

class EcoforestHeater {
  ...
  constructor(log, config) {
    ...
    this.pullTimer = new PullTimer(this.log, this.pullInterval, this.refreshEcoforestHeaterStatus.bind(this), () => {});
  ...
  refreshEcoforestHeaterStatus() {
    this.log.debug("Executing RefreshEcoforestHeaterStatus from:", this.apiroute);

    this.pullTimer.stop();
    this.httpRequest(this.apiroute, 'idOperacion=1002', function (error, response, responseBody) {
        if (error) {
          this.log.warn("Error while refreshEcoforestHeaterStatus: %s - %s", error.message, responseBody);
          this.pullTimer.start();
        } else if ( response.statusCode >= 300 ) {
            this.log.warn("Error while refreshEcoforestHeaterStatus: %d - %s - %s", response.statusCode, response.statusMessage, responseBody);
            this.pullTimer.start();
        } else {
  ...
  getServices() {
    ...
    this.refreshEcoforestHeaterStatus();
